...
class AdminTopicTagsControllerIT : BaseWebIT() {

    @Autowired
    lateinit var mockMvc: MockMvc

    @Autowired
    lateinit var useCase: AdminTopicTagsUseCase
    ...
    fun `POST -Add one valid entry with active=TRUE`() {
        ...
        val nameParam = "Test Topic Post 1"
    ...
    fun `POST -Add valid entry with active=FALSE (missing)`() {
        ...
        val nameParam = "Test Topic Post 2"
        val existCountBefore = useCase.getAll().count()
    ...
    fun `PUT -only change active from TRUE to FALSE (missing)`() {
        ...
        // Act: call the PUT endpoint with only id and no active value (missing)
        // Expect: HTTP status code, header HX-Redirect to admin page
        mockMvc.perform(
            put(endPoint)
                .contentType(MediaType.APPLICATION_FORM_URLENCODED)
    ...
    fun `PUT -change 'name' param with invalid`() {
        ...
        // Act: call the PUT endpoint with invalid name
        val mvcResult = mockMvc.perform(
            put(endPoint)
                .contentType(MediaType.APPLICATION_FORM_URLENCODED)
                .header("HX-Request", "true")
    ...
    fun `PUT -change 'description' param with invalid`() {
        ...
        // Act: call the PUT endpoint with invalid description
        val mvcResult = mockMvc.perform(
            put(endPoint)
                .contentType(MediaType.APPLICATION_FORM_URLENCODED)
                .header("HX-Request", "true")
    ...
    fun `PUT -change all params with invalid`() {
        ...
        // Act: call the PUT endpoint with invalid name and description
        val mvcResult = mockMvc.perform(
            put(endPoint)
                .contentType(MediaType.APPLICATION_FORM_URLENCODED)
                .header("HX-Request", "true")
